...
function cleanSessionCache() {
  try {
    ...
    if (deletedCount > 0) {
      logger.info(
        `Auto-clean session cache: Deleted ${deletedCount} old temporary Baileys session files.`,
      );
    }
  } catch (err) {
...
async function startBot() {
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      logger.warn(
        `Primary connection closed. Reason: ${lastDisconnect?.error?.message || "Unknown"}. Reconnecting: ${shouldReconnect}`,
      );
