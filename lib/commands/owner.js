    ...
    },
{
        ...
        run: async (sock, msg, args, { getTargetJid }) => {
            ...
            const normalizedJid = db.normalizeJid(target);
            if (db.isPrivilegedJid(normalizedJid)) {
            ...
            if (!db.data.users[normalizedJid]) {
                await sock.sendMessage(msg.key.remoteJid, { text: `⚠️ Pengguna @${normalizedJid.split('@')[0]} tidak ditemukan di database.`, mentions: [target] }, { quoted: msg });
                return;
            }
            delete db.data.users[normalizedJid];
            db.save();
            await sock.sendMessage(msg.key.remoteJid, { text: `🗑️ Berhasil menghapus @${normalizedJid.split('@')[0]} dari database.`, mentions: [target] }, { quoted: msg });
        }
    },
{
        name: 'demote',
