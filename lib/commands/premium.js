    ...
    },
{
        ...
        run: async (sock, msg, args, { sendTyping }) => {
            ...
            try {
                ...
                const isImage = mediaNode.imageMessage || (mediaNode.documentMessage && mediaNode.documentMessage.mimetype?.startsWith('image/'));
                ...
                if (isImage && text.trim()) {
                    try {
                        ...
                        const drawMemeText = (txt, y, isTop) => {
                            if (!txt) return;
                            txt = txt.toUpperCase();
                            
                            let fontSize = Math.floor(img.width * 0.09); // scale font size based on image width
                            ...
                            while (ctx.measureText(txt).width > maxWidth && fontSize > 12) {
                                fontSize -= 2;
                                ctx.font = `bold ${fontSize}px Impact, "Arial Black", Arial, sans-serif`;
                            }
                            ...
                            ctx.strokeText(txt, img.width / 2, y);
                            ctx.fillText(txt, img.width / 2, y);
                        };

                        const paddingY = img.height * 0.05;
                        drawMemeText(topText, paddingY, true);
                        drawMemeText(bottomText, img.height - paddingY, false);
