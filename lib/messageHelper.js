...
export async function sendInteractiveMessage(sock, jid, params, options = {}) {
    ...
    // 2. Prepare the interactive message payload
    ...
    // 3. Prepare the final message wrapper containing BOTH interactive message and fallback text.
    // If a client does not support interactiveMessage, WhatsApp's server/client will display the fallback text/caption.
    const messagePayload = {
        viewOnceMessage: {
            message: {
                ...interactiveMessage,
                // Appending text fallback inside the same payload for unsupported devices
                extendedTextMessage: {
                    text: fallbackText
                }
            }
    ...
    // 4. Send using relayMessage to bypass standard media validation errors
