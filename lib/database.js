    ...
    try {
      ...
      if (fs.existsSync(oldDbPath) && !fs.existsSync(dbPaths.users)) {
        try {
          ...
          if (parsed) {
            console.log(
              "[DB Migration] Migrating config/database.json data to split database files...",
            );
            if (parsed.users)
              fs.writeFileSync(
                dbPaths.users,
                JSON.stringify(parsed.users, null, 4),
                "utf8",
              );
            if (parsed.stats)
              fs.writeFileSync(
                dbPaths.command,
                JSON.stringify(parsed.stats, null, 4),
                "utf8",
              );
            if (parsed.groups)
              fs.writeFileSync(
                dbPaths.groups,
                JSON.stringify(parsed.groups, null, 4),
                "utf8",
              );
            if (parsed.settings?.jpmChannels)
              fs.writeFileSync(
                dbPaths.channels,
                JSON.stringify(parsed.settings.jpmChannels, null, 4),
                "utf8",
              );
            ...
            const premiumJids = Object.keys(parsed.users || {}).filter(
              (jid) => parsed.users[jid].premium,
            );
            fs.writeFileSync(
              dbPaths.premium,
