...
class Database {
    ...
    isPrivilegedJid(jid) {
        ...
        const normalizedOwner = this.normalizeJid(settings.ownerNumber).split('@')[0];

        if (normalizedJid === normalizedOwner) return true;

        // Also check bot admins list in db
        const admins = this.data.settings.admins || [];
        return admins.some(a => this.normalizeJid(a).split('@')[0] === normalizedJid);
    }
    ...
    writeToDisk() {
        try {
            ...
            // 1. Write users
            fs.writeFileSync(dbPaths.users, JSON.stringify(this.data.users, null, 4), 'utf8');
            ...
            fs.writeFileSync(dbPaths.command, JSON.stringify(this.data.stats, null, 4), 'utf8');

            // 5. Write JPM channels
            fs.writeFileSync(dbPaths.channels, JSON.stringify(this.data.settings.jpmChannels || [], null, 4), 'utf8');

            // 6. Write groups
            fs.writeFileSync(dbPaths.groups, JSON.stringify(this.data.groups || {}, null, 4), 'utf8');
    ...
    ensurePrivilegedUsers() {
        ...
        for (const jid of privilegedNumbers) {
            if (!this.data.users[jid]) {
                this.data.users[jid] = {
                    registered: true,
                    name: jid === this.normalizeJid(settings.ownerNumber) ? settings.ownerName : '',
                    banned: false,
                    premium: true,
                    limit: 100,
                    joinedAt: new Date().toISOString()
