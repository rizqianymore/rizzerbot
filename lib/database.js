...
class Database {
    ...
    load() {
        try {
            ...
            let ownerSettings = { owner: settings.ownerNumber, admins: [], selfMode: false, maintenance: false, registrationOpen: true };
    ...
    writeToDisk() {
        try {
            ...
            // 7. Write JPM blacklist
            this.safeWriteFileSync(dbPaths.blacklist, JSON.stringify(this.data.settings.jpmBlacklist || [], null, 4));
    ...
    getGroup(jid) {
        ...
        // Kembalikan null jika belum ada konfigurasi — mencegah auto-insert grup kosong
        return this.data.groups[jid] || null;
    }

    updateGroup(jid, props) {
        if (!this.data.groups) this.data.groups = {};
        // Baru inisialisasi saat ada perubahan nyata
