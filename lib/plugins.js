...
export async function reloadPluginFile(absolutePath, relativePath) {
    ...
    try {
        ...
        if (!loadedAny) {
            console.log(`  ! Plugin file ${relativePath} did not export any valid commands.`);
        }
    } catch (err) {
        console.error(`  ✗ Failed to load/reload plugin ${relativePath}: ${err.message}`);
    }
}
...
export async function loadPlugins() {
    ...
    updateDirectoryWatchers();
    console.log(`Watching plugins and case directories for automatic hot-reloads.`);
}
