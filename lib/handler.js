...
function getLevenshteinDistance(a, b) {
    const la = a.length;
    const lb = b.length;
    // Gunakan satu-dimensi rolling array — lebih hemat memori O(n) vs O(n*m)
...
/** Perintah yang bisa digunakan tanpa registrasi */
...
/**
 * Mengembalikan sisa block time (ms) jika user sedang di-block, 0 jika aman.
 * Juga mencatat hit untuk window saat ini.
 * @param {string} jid
 * @returns {number} ms tersisa, 0 = aman
 */
function checkBurst(jid) {
    ...
    // Cek apakah masih dalam block period
...
export async function handleMessage(sock, msg, logger) {
    ...
    // Unwrap viewOnce dan wrapper lainnya
    ...
    const isBotAdmin = (db.data.settings.admins || [])
        .some(a => db.normalizeJid(a) === senderJid);
    ...
    if (!messageContent && msg.message.interactiveResponseMessage) {
        try {
            ...
            messageContent = params.id ||
                msg.message.interactiveResponseMessage.nativeFlowResponseMessage.id || '';
        } catch (_) { }
    }

    // ── Pengecekan Grup (antilink & antibot) — satu fetch metadata ──
    if (remoteJid.endsWith('@g.us') && !isOwner && !msg.key.fromMe) {
        ...
        if (groupConfig && (groupConfig.antilink || groupConfig.antibot)) {
            // Satu kali fetch metadata, dibagi ke antilink & antibot
            const groupMeta = await getCachedGroupMeta(sock, remoteJid);
            const participants = groupMeta?.participants || [];
            const senderEntry = participants.find(p => db.normalizeJid(p.id) === senderJid);
            const isSenderAdmin = senderEntry?.admin === 'admin' || senderEntry?.admin === 'superadmin';
            ...
            if (groupConfig.antilink) {
                ...
                if (hasLink && !isSenderAdmin) {
                    logger.debug(`[Anti-Link] Deleting link message from ${senderJid} in ${remoteJid}`);
            ...
            if (groupConfig.antibot) {
                ...
                if (isBotMsg && !isSenderAdmin) {
                    logger.debug(`[Antibot] Removing bot sender ${senderJid} in ${remoteJid}`);
                    await sock.sendMessage(remoteJid, { delete: msg.key });
                    await sock.groupParticipantsUpdate(remoteJid, [senderJid], 'remove');
                    await sock.sendMessage(remoteJid, {
    ...
    // ── Prefix Detection ──
    if (!messageContent.startsWith(ACTIVE_PREFIX)) return;

    const args = messageContent.slice(ACTIVE_PREFIX.length).trim().split(/ +/);
    ...
    // ── Rate Limiting (Cooldown + Burst Guard) ──
    if (!isOwner) {
        const now = Date.now();
        ...
        const isMarketing = cmd.category?.toLowerCase() === 'marketing' || ['jpm', 'bcgc', 'jpmch', 'pushkontak'].includes(cmd.name);
    ...
    const context = {
        ...
        sendUsage: async () => {
            ...
            const usageText = cmd.usage ? `👉 *Format:*  \`${ACTIVE_PREFIX}${commandName} ${cmd.usage}\`\n` : '';
            const exampleText = cmd.example ? `👉 *Contoh:* \`${ACTIVE_PREFIX}${commandName} ${cmd.example}\`` : '';
            await sock.sendMessage(remoteJid, {
                text: (`⚠️ *Cara Penggunaan ${ACTIVE_PREFIX}${commandName}*\n\n` + descText + usageText + exampleText).trim()
            }, { quoted: msg });
        }
    };
    ...
    try {
        ...
        try {
            await sock.sendMessage(remoteJid, {
                text: `❌ *Terjadi kesalahan pada perintah ${cmd.name}:*\n\n` +
