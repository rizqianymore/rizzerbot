...
export async function handleMessage(sock, msg, logger) {
    ...
    if (prefixUsed !== null) {
        args = messageContent.slice(prefixUsed.length).trim().split(/ +/);
        commandName = args.shift()?.toLowerCase() || '';
    } else {
        // Pemanggilan tanpa prefix: cek jika kata pertama adalah nama perintah/alias yang terdaftar
        const tempArgs = messageContent.trim().split(/ +/);
    ...
    if (!cmd) {
        if (prefixUsed !== null) {
            ...
            for (const name of allCommands) {
                const distance = getLevenshteinDistance(commandName, name);
            ...
            // Suggest only if distance is small (threshold is 40% of command name length, min 2)
            const threshold = Math.max(2, Math.floor(commandName.length * 0.4));
    ...
    if (db.data.settings.maintenance && !isOwner) {
        await sock.sendMessage(remoteJid, { text: '⚠️ *RizzerBot sedang dalam pemeliharaan (maintenance).*' }, { quoted: msg });
        return;
    }
