...
export function tokenize(text) {
    ...
    // Reset regex index just to be safe
...
export function getEmojiUrl(emoji) {
    ...
    // Twemoji usually doesn't have fe0f suffix
...
export function measureTokensWidth(ctx, tokens, fontSize) {
    ...
    for (const token of tokens) {
        if (token.type === 'text') {
            ...
            // Emojis are treated as square elements matching or slightly exceeding the font size
            width += fontSize * 1.05;
        }
    }
    return width;
...
export function drawTokens(ctx, tokens, x, y, fontSize, emojiImages) {
    ...
    for (const token of tokens) {
        if (token.type === 'text') {
            ...
            // Align emoji vertically with the text (a bit offset to match the baseline alignment)
            const yOffset = (fontSize - size) / 2;
            if (img) {
                ctx.drawImage(img, currentX, y + yOffset, size, size);
            } else {
                // Fallback to text rendering of emoji if loading failed
                ctx.fillText(token.value, currentX, y);
            }
            currentX += size;
        }
...
export function drawTokensJustified(ctx, line, x, y, maxWidth, fontSize, emojiImages, isLastLine) {
