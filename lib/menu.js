...
│ ➩ ${activePrefix}ping
│ ➩ ${activePrefix}owner
│ ➩ ${activePrefix}runtime
│ ➩ ${activePrefix}checkuser
│ ➩ ${activePrefix}jkt84
...
export function getPluginsMenu() {
    ...
    const hardcodedCommands = new Set([
        ...
        'collage', 'rvo', 'qc', 'qr', 'statusdownloader', 'brat', 'bratvid',
    ...
    sortedCats.forEach(cat => {
        ...
        cmds.forEach((cmd) => {
            const aliases = cmd.aliases && cmd.aliases.length > 0
                ? ` _(alias: ${cmd.aliases.map(a => activePrefix + a).join(', ')})_`
                : '';
            const premBadge = cmd.premiumOnly ? ' *(Premium)*' : '';
            menuText += `│ ➩ *${activePrefix}${cmd.name}*${premBadge}${aliases}\n`;
        });
        menuText += `╰──────────────────\n\n`;
    });
