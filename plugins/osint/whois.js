...
export default {
    ...
    run: async (sock, msg, args, context) => {
        ...
        try {
            ...
            if (res.status !== 200) {
                if (res.status === 404) {
                    throw new Error('Domain tidak ditemukan atau tidak didukung oleh RDAP.');
                }
            ...
            if (Array.isArray(data.entities)) {
                const registrarEntity = data.entities.find(ent => ent.roles && ent.roles.includes('registrar'));
            ...
            let replyText = `🔍 *Domain WHOIS OSINT*\n\n` +
                            `• *Domain:* \`${ldhName}\`\n` +
                            `• *Registrar:* ${registrar}\n` +
