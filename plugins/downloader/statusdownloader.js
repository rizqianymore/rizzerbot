    ...
    try {
      ...
      if (quotedMedia.imageMessage) {
        messageContent = {
          document: buffer,
          mimetype: "image/jpeg",
          fileName: `Status_HD_${Date.now()}.jpg`,
          caption:
            "✅ Berhasil didownload (Resolusi Asli/HD)!\n⚡ _Via Kyros-MD API_",
        };
      } else if (quotedMedia.videoMessage) {
        messageContent = {
          document: buffer,
          mimetype: "video/mp4",
          fileName: `Status_Video_HD_${Date.now()}.mp4`,
          caption:
            "✅ Berhasil didownload (Resolusi Asli/HD)!\n⚡ _Via Kyros-MD API_",
        };
        ...
        messageContent = {
          document: buffer,
          mimetype: "audio/mp4",
          fileName: `Status_Audio_${Date.now()}.m4a`,
          caption: "✅ Audio berhasil didownload\n⚡ _Via Kyros-MD API_",
        };
      } else if (quotedMedia.documentMessage) {
        messageContent = {
          document: buffer,
          mimetype: quotedMedia.documentMessage.mimetype,
          fileName: quotedMedia.documentMessage.title || "document",
      ...
      await sock.sendMessage(
        msg.key.remoteJid,
        {
          text: "❌ Gagal mendownload status. Pastikan media belum kedaluwarsa atau sudah diunduh di HP.",
        },
        { quoted: msg },
      );
    }
