...
export default [
    {
        ...
        run: async (sock, msg, args, { sendTyping, senderJid }) => {
            ...
            const getMediaNode = (m) => {
                ...
                const hasImage = keys.includes('imageMessage') || (keys.includes('documentMessage') && content.documentMessage.mimetype?.startsWith('image/'));
            ...
            if (!directMedia && !quotedMedia) {
                await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Kirim atau balas gambar dengan perintah *.addkolase* untuk menambahkannya ke sesi.' }, { quoted: msg });
                return;
            }
            ...
            try {
                ...
                if (directMedia) {
                    ...
                    mediaMessage = {
                        key: { remoteJid: msg.key.remoteJid, id: quotedInfo.stanzaId, participant: quotedInfo.participant, fromMe: false },
                        message: quotedMedia
                    };
                }
                ...
                if (session.length >= 9) {
                    await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Maksimal 9 gambar yang bisa ditambahkan. Ketik *.kolase* untuk menggabungkannya.' }, { quoted: msg });
                    return;
                }

                session.push(buffer);
                await sock.sendMessage(msg.key.remoteJid, { text: `✅ Gambar berhasil ditambahkan! [Total: ${session.length}/9]\nKirim gambar lain dengan *.addkolase* atau ketik *.kolase* untuk memproses.` }, { quoted: msg });
            } catch (err) {
    ...
    },
    {
        ...
        run: async (sock, msg, args, { sendTyping, senderJid }) => {
            ...
            if (!session || session.length === 0) {
                await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Anda belum menambahkan gambar. Kirim atau balas gambar dengan perintah *.addkolase* terlebih dahulu.' }, { quoted: msg });
                return;
            }
            ...
            try {
                ...
                ctx.quadraticCurveTo(overlayMargin + overlayW, overlayMargin, overlayMargin + overlayW, overlayMargin + radius);
                ctx.lineTo(overlayMargin + overlayW, overlayMargin + overlayH - radius);
                ctx.quadraticCurveTo(overlayMargin + overlayW, overlayMargin + overlayH, overlayMargin + overlayW - radius, overlayMargin + overlayH);
                ctx.lineTo(overlayMargin + radius, overlayMargin + overlayH);
                ctx.quadraticCurveTo(overlayMargin, overlayMargin + overlayH, overlayMargin, overlayMargin + overlayH - radius);
                ctx.lineTo(overlayMargin, overlayMargin + radius);
                ctx.quadraticCurveTo(overlayMargin, overlayMargin, overlayMargin + radius, overlayMargin);
                ctx.closePath();
                ctx.fill();
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    ctx.quadraticCurveTo(cellX + cellW, cellY, cellX + cellW, cellY + cellRadius);
                    ctx.lineTo(cellX + cellW, cellY + cellH - cellRadius);
                    ctx.quadraticCurveTo(cellX + cellW, cellY + cellH, cellX + cellW - cellRadius, cellY + cellH);
                    ctx.lineTo(cellX + cellRadius, cellY + cellH);
                    ctx.quadraticCurveTo(cellX, cellY + cellH, cellX, cellY + cellH - cellRadius);
                ...
                await sock.sendMessage(msg.key.remoteJid, { text: '❌ Gagal membuat kolase.' }, { quoted: msg });
            }
        }
    },
    {
        ...
        run: async (sock, msg, args, { senderJid }) => {
            if (collageSessions.has(senderJid)) {
                collageSessions.delete(senderJid);
                await sock.sendMessage(msg.key.remoteJid, { text: '✅ Sesi pembuatan kolase telah dibatalkan.' }, { quoted: msg });
            } else {
                await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Anda tidak sedang membuat kolase.' }, { quoted: msg });
            }
        }
    }
];
