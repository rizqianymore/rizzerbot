  ...
  run: async (sock, msg, args, { sendTyping }) => {
    ...
    if (!directMedia && !quotedMedia) {
      await sock.sendMessage(
        msg.key.remoteJid,
        {
          text: "⚠️ Silakan kirim atau balas gambar/video/stiker/dokumen media dengan perintah *.sticker*",
        },
        { quoted: msg },
      );
      return;
    ...
    try {
      ...
      try {
        ...
        const extractedEmojis = [
          ...new Set(
            tokens.filter((t) => t.type === "emoji").map((t) => t.value),
          ),
        ];
        const emojiParam =
