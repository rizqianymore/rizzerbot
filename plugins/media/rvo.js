export default {
    name: 'rvo',
    description: 'Membaca / mengambil kembali media pesan sekali lihat (View Once).',
    ...
    run: async (sock, msg, args, { sendTyping }) => {
        ...
        const getMediaNode = (m) => {
            ...
            if (keys.includes('viewOnceMessageV2Extension')) return getMediaNode(content.viewOnceMessageV2Extension.message);
        ...
        if (!quotedMsg) {
            await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Balas pesan sekali lihat (View Once) yang ingin Anda ambil medianya.' }, { quoted: msg });
            return;
        }

        const mediaNode = getMediaNode(quotedMsg);
        if (!mediaNode) {
            await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Pesan yang Anda balas bukan merupakan media sekali lihat (View Once).' }, { quoted: msg });
            return;
        }
        ...
        try {
            ...
            if (mediaNode.imageMessage) {
                ...
                await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ Tipe media tidak didukung.' }, { quoted: msg });
            }
        } catch (err) {
            console.error('[RVO Error]', err);
            await sock.sendMessage(msg.key.remoteJid, { text: `❌ Gagal mengambil media sekali lihat: ${err.message}` }, { quoted: msg });
        }
    }
};
